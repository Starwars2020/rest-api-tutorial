...
podTemplate(
    ...
    node(label) {
        def githubRepository = "https://github.com/Starwars2020/rest-api-tutorial.git"
        def githubCredential = "github-api-token"
        def dockerRegistry = "localhost:5000"
        def dockerRegistryCredential = ""
        def dockerImageName = "localhost:5000/rest-api-tutorial"
        ...
        stage('Run kubectl') {
            container('kubectl') {
                ...
                sh "kubectl apply -f rest-api-tutorial-ingress.yaml -n ${nameSpace} --validate=flase"
            }
        }
    }
}